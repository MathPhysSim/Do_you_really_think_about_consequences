...
class NormalizedEnv(gym.Wrapper):
    """
    A gym environment wrapper that normalizes the observation and action spaces to [0, 1].

    The observations are transformed as:
        obs_normalized = (obs - obs_low) / (obs_high - obs_low)
    And actions provided in normalized space are mapped back to the original range:
        action = action_normalized * (act_high - act_low) + act_low
    """
    ...
    def __init__(self, env: gym.Env):
        ...
        # Define new normalized observation and action spaces.
        self.observation_space = gym.spaces.Box(
            low=0.0,
            high=1.0,
            shape=self._orig_obs_space.shape,
            dtype=np.float32
        )
        self.action_space = gym.spaces.Box(
            low=0.0,
            high=1.0,
            shape=self._orig_action_space.shape,
            dtype=np.float32
        )
    ...
    def step(self, action: np.ndarray):
        # Denormalize action before passing it to the underlying env.
        action_denorm = self._denormalize_action(action)
        obs, reward, done, truncated, info = self.env.step(action_denorm)
        return self._normalize_obs(obs), reward, done, truncated, info

    def _normalize_obs(self, obs: np.ndarray) -> np.ndarray:
        # Normalize observations from original range to [0, 1]
        low = self._orig_obs_space.low
        high = self._orig_obs_space.high
        # Avoid division by zero in degenerate cases:
        return (obs - low) / (high - low + 1e-8)

    def _denormalize_action(self, action: np.ndarray) -> np.ndarray:
        # Map the normalized action back to the original action space.
        low = self._orig_action_space.low
        high = self._orig_action_space.high
...
def load_env_config(env_config: str = 'config/environment_setting.yaml') -> Any:
    ...
    # Wrap the environment with NormalizedEnv so that observations and actions are in [0,1].
