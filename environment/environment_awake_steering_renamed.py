...
class AwakeSteering(gym.Env):
    ...
    def setup_dimensions(self):
        ...
        # print(f'self.rmatrix: {self.rmatrix}')
        # Use pseudo-inverse for numerical stability
        self.rmatrix_inverse = np.linalg.pinv(self.rmatrix)
        # print(f'self.rmatrix_inverse: {self.rmatrix_inverse}')
    ...
    def reset(
        ...
        self.state  = self.find_good_initialisation()
        return_state = self.state.copy()
        return return_state, {}

    def find_good_initialisation(self):
        """Finds a good initial state within defined thresholds."""
        nr_init_trials_to_find_a_good_setting = 10000
        for _ in range(nr_init_trials_to_find_a_good_setting):
            init_state = self.rmatrix.dot(self.action_space.sample())
